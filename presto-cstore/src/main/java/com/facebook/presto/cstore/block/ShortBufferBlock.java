...
public final class ShortBufferBlock
        implements Block
{
    private static final int INSTANCE_SIZE = ClassLayout.parseClass(ShortBufferBlock.class).instanceSize();

    private final ShortBuffer value;
    @Nullable
    ...
    public ShortBufferBlock(ShortBuffer value, @Nullable Bitmap valueIsNull)
    {
        this(value, valueIsNull, value.capacity(), 0);
    }

    private ShortBufferBlock(ShortBuffer value, @Nullable Bitmap valueIsNull, int positionCount, int offsetBase)
    {
        this.value = value;
    ...
    public Block getRegion(int positionOffset, int length)
    {
        ...
        return new ShortBufferBlock(value, valueIsNull, length, offsetBase + positionOffset);
    }
    ...
    public Block copyRegion(int position, int length)
    {
        ...
        for (int i = 0; i < length; i++) {
            newValues[i] = getShortUnchecked(i + offsetBase + position);
        }
        return new ShortArrayBlock(length, Optional.ofNullable(newValueIsNull), newValues);
    }
