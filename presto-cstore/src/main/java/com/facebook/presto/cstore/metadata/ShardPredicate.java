...
import static com.facebook.presto.cstore.storage.ShardStats.truncateIndexValue;
import static com.facebook.presto.cstore.util.UuidUtil.uuidStringToBytes;
...
class ShardPredicate
{
    ...
    public static ShardPredicate create(TupleDomain<CStoreColumnHandle> tupleDomain)
    {
        ...
        for (Entry<CStoreColumnHandle, Domain> entry : tupleDomain.getDomains().get().entrySet()) {
            Domain domain = entry.getValue();
            if (domain.isNullAllowed() || domain.isAll()) {
                continue;
            }
            CStoreColumnHandle handle = entry.getKey();
