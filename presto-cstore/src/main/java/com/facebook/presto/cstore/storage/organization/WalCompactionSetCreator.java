...
import static com.facebook.presto.cstore.storage.organization.ShardOrganizerUtil.createOrganizationSet;
import static com.facebook.presto.cstore.storage.organization.ShardOrganizerUtil.createWalOrganizationSet;
import static com.facebook.presto.cstore.storage.organization.ShardOrganizerUtil.getShardsByDaysBuckets;
import static com.google.common.base.Preconditions.checkArgument;
import static java.util.Comparator.comparing;
import static java.util.Objects.requireNonNull;
import static java.util.stream.Collectors.toCollection;

public class WalCompactionSetCreator
{
    ...
    public WalCompactionSetCreator(TemporalFunction temporalFunction, DataSize maxShardSize, long maxShardRows,
    ...
    public Set<WalCompactionSet> createCompactionSets(Table tableInfo, Collection<ShardIndexInfo> shards, UUID walUuid, long walBaseOffset, long transactionEnd)
    {
        Collection<Collection<ShardIndexInfo>> shardsByDaysBuckets = getShardsByDaysBuckets(tableInfo, shards, temporalFunction);

        ImmutableSet.Builder<WalCompactionSet> compactionSets = ImmutableSet.builder();
        for (Collection<ShardIndexInfo> shardInfos : shardsByDaysBuckets) {
            compactionSets.addAll(buildCompactionSets(tableInfo, ImmutableSet.copyOf(shardInfos), walUuid, walBaseOffset, transactionEnd));
        }
        return compactionSets.build();
    }

    private Set<WalCompactionSet> buildCompactionSets(Table tableInfo, Set<ShardIndexInfo> shardIndexInfos, UUID walUuid, long walBaseOffset, long transactionEnd)
    {
        ...
        for (ShardIndexInfo shard : shards) {
            if (((consumedBytes + shard.getUncompressedSize()) > maxShardSize.toBytes()) ||
                ...
                addToCompactionSets(compactionSets, shardsToCompact, tableId, tableInfo, priority, walUuid, walBaseOffset, transactionEnd);
    ...
    private void addToCompactionSets(ImmutableSet.Builder<WalCompactionSet> compactionSets, Set<ShardIndexInfo> shardsToCompact, long tableId, Table tableInfo, int priority,
            UUID walUuid, long walBaseOffset, long transactionEnd)
    {
        ...
        if (shardsToCompact.size() >= minShardsToCompactSize || shardsToCompact.stream().anyMatch(shard -> shard.getDeltaUuid().isPresent())) {
            compactionSets.add(createWalOrganizationSet(tableId, shardsToCompact, priority, walUuid, walBaseOffset, transactionEnd));
        }
    }
