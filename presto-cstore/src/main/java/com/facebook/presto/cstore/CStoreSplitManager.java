...
import com.facebook.presto.cstore.metadata.ShardNodes;
import com.facebook.presto.cstore.util.SynchronizedResultIterator;
        ...
        implements ConnectorSplitManager
{
    ...
    @Inject
    public CStoreSplitManager(CStoreConnectorId connectorId, NodeSupplier nodeSupplier, ShardManager shardManager, BackupService backupService)
    {
        this(connectorId, nodeSupplier, shardManager, requireNonNull(backupService, "backupService is null").isBackupAvailable());
    }

    public CStoreSplitManager(CStoreConnectorId connectorId, NodeSupplier nodeSupplier, ShardManager shardManager, boolean backupAvailable)
    {
    ...
    {
        CStoreTableLayoutHandle handle = (CStoreTableLayoutHandle) layout;
        CStoreTableHandle table = handle.getTable();
        TupleDomain<CStoreColumnHandle> effectivePredicate = toRaptorTupleDomain(handle.getConstraint());
    ...
    private static TupleDomain<CStoreColumnHandle> toRaptorTupleDomain(TupleDomain<ColumnHandle> tupleDomain)
    {
        return tupleDomain.transform(handle -> (CStoreColumnHandle) handle);
    }
