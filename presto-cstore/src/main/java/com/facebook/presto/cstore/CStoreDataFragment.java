...
public class CStoreDataFragment
{
    private static final JsonCodec<CStoreDataFragment> MEMORY_DATA_FRAGMENT_CODEC = jsonCodec(CStoreDataFragment.class);

    private final HostAddress hostAddress;
    private final long rows;

    @JsonCreator
    public CStoreDataFragment(
    ...
    public static CStoreDataFragment fromSlice(Slice fragment)
    {
        return MEMORY_DATA_FRAGMENT_CODEC.fromJson(fragment.getBytes());
    }

    public static CStoreDataFragment merge(CStoreDataFragment a, CStoreDataFragment b)
    {
        checkArgument(a.getHostAddress().equals(b.getHostAddress()), "Can not merge fragments from different hosts");
        return new CStoreDataFragment(a.getHostAddress(), a.getRows() + b.getRows());
    }
}
