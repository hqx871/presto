...
public final class BinaryBufferBlock
        implements Block
{
    private static final int INSTANCE_SIZE = ClassLayout.parseClass(BinaryBufferBlock.class).instanceSize();

    private final IntBuffer offset;
    ...
    public BinaryBufferBlock(ByteBuffer value, @Nullable Bitmap valueIsNull, IntBuffer offset)
    {
        this(value, valueIsNull, offset, value.capacity(), 0);
    }

    private BinaryBufferBlock(ByteBuffer value, @Nullable Bitmap valueIsNull, IntBuffer offset, int positionCount, int offsetBase)
    {
        this.value = value;
    ...
    {
        checkReadablePosition(position);
        return getByteUnchecked(position);
    }
    ...
    public Block getRegion(int positionOffset, int length)
    {
        ...
        return new BinaryBufferBlock(value, valueIsNull, offset, length, offsetBase + positionOffset);
    }
    ...
    public Block copyRegion(int position, int length)
    {
        ...
        for (int i = 0; i < length; i++) {
            newValues[i] = getByteUnchecked(i + offsetBase + position);
        }
        return new ByteArrayBlock(length, Optional.ofNullable(newValueIsNull), newValues);
    }
